...
export function createDatasetFilters (filters) {
  return function (dataset) {
    ...
    if (filters.decade) {
      conditions.push(dataset.decade && slugify(dataset.decade).indexOf(filters.decade) !== -1)
    }
    if (filters.country) {
      conditions.push(dataset.country && slugify(dataset.country).indexOf(filters.country) !== -1)
    }
    if (filters.access) {
